...
### Данные/прочее
- `supabase_schema.sql` — схема БД. `migrations/` — миграции (применять через Supabase MCP). `tests/` — pytest (`test_lessons`,`test_billing`,`test_series_view`), запуск `python3 -m pytest -q`. `prompts/` — промпты ИИ. `scripts/` — вспомогательное.
