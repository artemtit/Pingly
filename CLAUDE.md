...
- Цена: 500–1000 ₽/мес
- **Весь функционал — на сайте, бот — чисто служебный (для обеих ролей).** Бот умеет ровно три вещи: (1) вход/регистрация через Telegram + выдача ссылки в кабинет (`/start`, `/web`); (2) доставка напоминаний ученикам; (3) кнопки «Буду/Отменяю» под напоминанием (`lesson_confirm`/`lesson_cancel`, «Отменяю» пишет статус в БД — репетитор видит на сайте). В боте НЕТ управляющих меню/FSM (ни у репетитора, ни у ученика) — всё это убрано осознанно (handlers/keyboards.py удалён). Управление учениками/расписанием/ДЗ — только в веб-кабинете.
