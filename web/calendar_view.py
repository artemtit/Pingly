...
from application.services.timezones import current_tz
...
STATUS_LABELS = {
    ...
    "reschedule_requested": "🟠 Просьба перенести",
    "completed": "🔵 Проведено",
...
def _parse(raw: object) -> datetime | None:
    if not raw:
        return None
    try:
        return datetime.fromisoformat(str(raw).replace("Z", "+00:00")).astimezone(current_tz())
    except ValueError:
        return None
